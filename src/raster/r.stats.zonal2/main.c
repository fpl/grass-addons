...
int main(int argc, char **argv)
{
    ...
    struct {
	struct Option *method, *basemap, *covermap, *output, *rules;
    } opt;
    struct {
	struct Flag *c, *r;
    } flag;
    char methods[2048];
    const char *basemap, *covermap, *output, *rules;
    int usecats;
    int reclass;
	...
	opt.rules->description = _("Reclassification rules file");
	...
	fp = fopen(tempfile, "w");
	if (!fp)
	    G_fatal_error(_("Unable to open rules file"));
